...
/**
 * `scope` is the security-critical option: a row outside the scope must be
 * invisible to *every* operation, not merely absent from the list. If it 404s on
 * list but can still be fetched by id — or worse, updated or deleted — the scope
 * is decoration, not security.
 */
async function scoped(): Promise<TestClient> {
  const client = await setup({
    ...open,
    scope: (q) => q.where("kind", "public"),
  });

  await connection().write("INSERT INTO items (name, kind) VALUES ('visible', 'public')", []);
  await connection().write("INSERT INTO items (name, kind) VALUES ('hidden', 'private')", []);
  return client;
}

test("apiResource: a scoped-out row is invisible to list", async () => {
  ...
  assert.deepEqual(
    body.data.map((i) => i.name),
    ["visible"],
  );
});

test("apiResource: a scoped-out row cannot be READ by id — it 404s", async () => {
  const client = await scoped();

  (await client.get("/items/1")).assertOk(); // in scope
  (await client.get("/items/2")).assertNotFound(); // out of scope — not 403, not 200
});

test("apiResource: a scoped-out row cannot be UPDATED", async () => {
  const client = await scoped();

  (await client.put("/items/2", { name: "pwned", kind: "private" })).assertNotFound();

  // ...and it really wasn't touched.
  const rows = await connection().select("SELECT name FROM items WHERE id = 2", []);
  assert.equal((rows[0] as { name: string }).name, "hidden");
});

test("apiResource: a scoped-out row cannot be DELETED", async () => {
  const client = await scoped();

  (await client.delete("/items/2")).assertNotFound();

  const rows = await connection().select("SELECT id FROM items WHERE id = 2", []);
  assert.equal(rows.length, 1, "the row must still be there");
});

/* ------------------------------- beforeWrite ------------------------------ */

test("apiResource: beforeWrite can set fields the client never sends", async () => {
  const client = await setup({
    ...open,
    beforeWrite: (data, _c, action) => ({ ...data, kind: action === "create" ? "seeded" : "touched" }),
  });

  const created = await client.post("/items", { name: "a", kind: "ignored" });
  created.assertCreated().assertJsonContains({ data: { kind: "seeded" } });

  const updated = await client.put("/items/1", { name: "a", kind: "ignored" });
  updated.assertOk().assertJsonContains({ data: { kind: "touched" } });
});

/* -------------------------------- transform ------------------------------- */

test("apiResource: transform shapes the output for both list and item", async () => {
  const client = await setup({
    ...open,
    transform: (model) => ({ label: (model as unknown as Item).name.toUpperCase() }),
  });

  await client.post("/items", { name: "ada", kind: "x" });

  (await client.get("/items/1")).assertJsonContains({ data: { label: "ADA" } });
  (await client.get("/items")).assertJsonContains({ data: [{ label: "ADA" }] });
});

/* ------------------------------- pagination ------------------------------- */

test("apiResource: list paginates and reports meta", async () => {
  const client = await setup({ ...open, perPage: 2 });

  for (const name of ["a", "b", "c", "d", "e"]) {
    await connection().write("INSERT INTO items (name, kind) VALUES (?, 'k')", [name]);
  }

  const first = await client.get("/items?sort=id");
  first.assertOk().assertJsonContains({
    meta: { total: 5, perPage: 2, currentPage: 1, lastPage: 3 },
  });
  assert.equal(first.json<{ data: unknown[] }>().data.length, 2);

  const second = await client.get("/items?sort=id&page=2");
  assert.deepEqual(
    second.json<{ data: Array<{ name: string }> }>().data.map((i) => i.name),
    ["c", "d"],
  );
});

test("apiResource: ?perPage cannot exceed maxPerPage — no 'give me everything'", async () => {
  const client = await setup({ ...open, perPage: 2, maxPerPage: 3 });

  for (const name of ["a", "b", "c", "d", "e"]) {
    await connection().write("INSERT INTO items (name, kind) VALUES (?, 'k')", [name]);
  }

  const res = await client.get("/items?perPage=9999");
  res.assertJsonContains({ meta: { perPage: 3 } });
  assert.equal(res.json<{ data: unknown[] }>().data.length, 3);
});

/* --------------------------- the openapi contract ------------------------- */

test("the openapi config key stays in sync with the openapi package", async () => {
  // resource.ts deliberately hardcodes this key rather than importing it, so the
  // api package never depends on openapi. That's the right call — but it makes the
  // two free to drift, and a drift would silently stop documenting every generated
  // route. A comment can't catch that; this can.
  const { OPENAPI_KEY } = await import("../src/openapi/doc.js");

  const app = new Application();
  await app.boot([], { discoverConfig: false, config: { app: {} } });
  const router = app.make(Router);
  apiResource(router, Item, { ...open, path: "things" });

  const route = router.all().find((r) => r.name === "things.list")!;
  assert.ok(
    OPENAPI_KEY in route.config,
    `apiResource writes its docs under a key the openapi package doesn't read ` +
      `(expected "${OPENAPI_KEY}", got ${JSON.stringify(Object.keys(route.config))}).`,
  );
});
