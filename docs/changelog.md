...
- **2FA setup no longer goes blank after a bad code** — `pendingTwoFactorSetup()`
  rebuilds the local QR from the stored secret; starters keep the scanner on
  screen. QR data URLs use base64 for broader `<img>` compatibility.

[0.83.13]: https://github.com/shaferllc/keel/releases/tag/v0.83.13
...
- **Local 2FA QR codes** — `enableTwoFactor()` returns `qr` (SVG `data:` URL via
  `uqr`). Starters render an `<img>` so the otpauth secret never hits a CDN.
  Also exported: `otpauthQrSvg` / `otpauthQrDataUrl`.

[0.83.11]: https://github.com/shaferllc/keel/releases/tag/v0.83.11
